...
describe("Cluster Module", () => {
  ...
  const mockPeer: PeerInfo = createTestPeerInfo({
    id: "peer-123",
    name: "Test User",
  });
  ...
  describe("announcePresence", () => {
    ...
    it("should handle errors gracefully", async () => {
      ...
      await expect(
        announcePresence(mockDatabase, mockDocId, mockPeer)
  ...
  describe("stopAnnouncingPresence", () => {
    ...
    it("should not throw on cleanup errors", async () => {
      ...
      await expect(
        stopAnnouncingPresence(mockDatabase, mockDocId, mockPeer.id)
  ...
  describe("cleanupStalePeers", () => {
    ...
    it("should remove signaling data for stale peers", async () => {
      ...
      const refCalls = vi.mocked(ref).mock.calls;
      const signalingPath = refCalls.find(
        (call) => call[1] && call[1].includes("signaling")
      );
    ...
    it("should not throw on cleanup errors", async () => {
      ...
      await expect(
        cleanupStalePeers(mockDatabase, mockDocId)
      ).resolves.toBeUndefined();
    });
